...
# LOAD DATA FILES
# ------------------------
sym_des     = read_csv_any("datasets/symptoms_clean_final.csv")
precautions = read_csv_any("datasets/precaution_clean_final.csv")
description = read_csv_any("datasets/description_clean_final.csv")
medications = read_csv_any("datasets/medication_clean_final.csv")
diets       = read_csv_any("datasets/diet_clean_final.csv")

try:
    workout = read_csv_any("datasets/workout_clean_final.csv")
except:
    workout = pd.DataFrame(columns=["disease", "workout"])

training_df = read_csv_any("datasets/Training_medical_FULL_clean.csv")
...
# LOAD MODEL
# ------------------------
svc_path = load_any("models/svc.pkl")
...
def get_predicted_value(patient_symptoms):
    ...
    # Feature order exactly same as model training
...
def predict():
    ...
    if len(user_symptoms) < 3:
        return render_template(
            "index.html",
            message="⚠️ Please enter at least 3 symptoms for accurate prediction"
        )
    ...
    try:
        ...
        if predicted_disease is None:
            return render_template(
                "index.html",
                message="⚠️ Not enough information. Please add more symptoms."
            )
